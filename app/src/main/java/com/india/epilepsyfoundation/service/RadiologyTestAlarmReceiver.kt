...
class RadiologyTestAlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (alarmIndex == -1) {
            Log.e("RadiologyAlarm", "❌ Received alarm with invalid or missing 'alarm_index'! Aborting...")
            return
        }

        val notificationManager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        val channelId = "Radiology_visit_channel_v2"

        val title = localizedContext.getString(R.string.radiology_test_reminder_title)
        val text = if (labName.isNotEmpty() && visitDate.isNotEmpty() && visitTime.isNotEmpty()) {
            localizedContext.getString(R.string.radiology_test_reminder_text, testName, labName, visitDate, visitTime)
        } else {
            localizedContext.getString(R.string.radiology_test_reminder_text_short, testName)
        }
