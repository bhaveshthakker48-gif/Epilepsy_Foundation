...
import com.india.epilepsyfoundation.adapter.PathologyTestReminderAdapter
import com.india.epilepsyfoundation.databinding.ActivityPathologyTestReminderBinding
...
class PathologyTestReminderActivity : BaseActivity() {

    private lateinit var binding: ActivityPathologyTestReminderBinding
    private val viewModel: ReminderViewmodel by viewModels()
    private lateinit var adapter: PathologyTestReminderAdapter

    override fun onCreate(savedInstanceState: Bundle?) {
        setLocale(getCurrentLanguage())
        super.onCreate(savedInstanceState)
        binding = ActivityPathologyTestReminderBinding.inflate(LayoutInflater.from(this))
        ...
        titleTextView.text = getString(R.string.title_pathology_test)

        binding.toolbar.addView(customView)

        adapter = PathologyTestReminderAdapter(
            emptyList(),
            onDeleteClick = { visit -> showDeleteConfirmationDialog(visit) },
            onEditClick = { visit -> showEditDialog(visit) }
        )
        binding.pathologyRecycleView.adapter = adapter
        binding.pathologyRecycleView.layoutManager = LinearLayoutManager(this)

        fetchDoctorVisits()
    }

    private fun showEditDialog(visit: PathologyTestReminderEntity) {
        ...
        val alertDialog = AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.save)) { _, _ ->
                ...
                viewModel.updatePathologyTestData(updatedVisit) {
                    fetchDoctorVisits()
    ...
    private fun showDeleteConfirmationDialog(visit: PathologyTestReminderEntity) {
        AlertDialog.Builder(this)
            .setTitle(getString(R.string.delete_confirmation_title))
            .setMessage(getString(R.string.delete_confirmation_message))
            .setPositiveButton(getString(R.string.yes)) { _, _ ->
                viewModel.deletePathologyTestReminder(visit) {
                    fetchDoctorVisits()
                }
            }
    ...
    private fun fetchDoctorVisits() {
        viewModel.getAllPathologyTestReminder { visits ->
            if (visits.isEmpty()) {
                binding.txtNoData.visibility = View.VISIBLE
                binding.pathologyRecycleView.visibility = View.GONE
            } else {
                binding.txtNoData.visibility = View.GONE
                binding.pathologyRecycleView.visibility = View.VISIBLE
