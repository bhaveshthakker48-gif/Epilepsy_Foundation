...
import com.india.epilepsyfoundation.adapter.MedicationReminderAdapter
import com.india.epilepsyfoundation.databinding.ActivityMedicationReminderBinding
...
class MedicationReminderActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMedicationReminderBinding.inflate(LayoutInflater.from(this))
        ...
        titleTextView.text = getString(R.string.title_medication)
        ...
        binding.medicationRecycleView.adapter = adapter
        binding.medicationRecycleView.layoutManager = LinearLayoutManager(this)

        fetchDoctorVisits()
    }

    private fun showEditDialog(visit: MedicationReminderEntity) {
        val dialogBinding = LayoutInflater.from(this).inflate(R.layout.dialog_edit_medication_reminder, null)
        ...
        val durationInput = dialogBinding.findViewById<EditText>(R.id.etDuration)
        val durationUnitInput = dialogBinding.findViewById<EditText>(R.id.etDurationUnit)
        val frequencyInput = dialogBinding.findViewById<EditText>(R.id.etFrequency)
        val doseInput = dialogBinding.findViewById<EditText>(R.id.etDose)

        // Pre-fill values
        doctorNameInput.setText(visit.medicationName)
        visitDateInput.setText(visit.startDate)
        visitTimeInput.setText(visit.startTime)
        durationInput.setText(visit.duration)
        durationUnitInput.setText(visit.durationUnit)
        frequencyInput.setText(visit.frequency)
        doseInput.setText(visit.dose)

        visitDateInput.inputType = InputType.TYPE_NULL
        visitTimeInput.inputType = InputType.TYPE_NULL
        durationUnitInput.inputType = InputType.TYPE_NULL
        frequencyInput.inputType = InputType.TYPE_NULL

        visitDateInput.setOnClickListener { showDatePickerDialog(visitDateInput) }
        visitTimeInput.setOnClickListener { showTimePickerDialog(visitTimeInput) }

        durationUnitInput.setOnClickListener {
            showSingleChoiceDialog(
                ...
                durationUnitInput.setText(selected)
            }
        }
        ...
        frequencyInput.setOnClickListener {
            showSingleChoiceDialog(
                ...
                frequencyInput.setText(selected)
            }
        }
        ...
        val alertDialog = AlertDialog.Builder(this)
            ...
            .setPositiveButton(getString(R.string.save)) { _, _ ->
                val updatedVisit = visit.copy(
                    medicationName = doctorNameInput.text.toString(),
                    startDate = visitDateInput.text.toString(),
                    startTime = visitTimeInput.text.toString(),
                    duration = durationInput.text.toString(),
                    durationUnit = durationUnitInput.text.toString(),
                    frequency = frequencyInput.text.toString(),
                    dose = doseInput.text.toString()
                )
                viewModel.updateReminderMedicationData(updatedVisit) {
                    fetchDoctorVisits()
        ...
        // Set custom background color (example: light purple)
    ...
    private fun showDeleteConfirmationDialog(visit: MedicationReminderEntity) {
        AlertDialog.Builder(this)
            .setTitle(getString(R.string.delete_confirmation_title))
            .setMessage(getString(R.string.delete_confirmation_message))
            .setPositiveButton(getString(R.string.yes)) { _, _ ->
                viewModel.deleteMedicationReminder(visit) {
                    fetchDoctorVisits()
                }
            }
    ...
    private fun fetchDoctorVisits() {
        viewModel.getAllMedicationReminder { visits ->
            if (visits.isEmpty()) {
                binding.txtNoData.visibility = View.VISIBLE
                binding.medicationRecycleView.visibility = View.GONE
            } else {
                binding.txtNoData.visibility = View.GONE
                binding.medicationRecycleView.visibility = View.VISIBLE
