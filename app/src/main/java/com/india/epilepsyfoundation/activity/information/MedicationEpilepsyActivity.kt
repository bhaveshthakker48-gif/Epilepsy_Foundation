...
import com.india.epilepsyfoundation.databinding.ActivityMedicationEpilepsyBinding
import com.india.epilepsyfoundation.utils.SharedPreference
import java.util.Locale

class MedicationEpilepsyActivity : BaseActivity() {

    private lateinit var binding: ActivityMedicationEpilepsyBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMedicationEpilepsyBinding.inflate(LayoutInflater.from(this))
        ...
        titleTextView.text = getString(R.string.lbl_epi_medication)

        binding.toolbar.addView(customView)

        val combinedHtml = getString(R.string.txt_epi_medication)

        // Set HTML text to TextView
        binding.instruction.text = HtmlCompat.fromHtml(combinedHtml, HtmlCompat.FROM_HTML_MODE_LEGACY)
    ...
    private fun playAudioForCurrentLanguage() {
        val audioResId = when (getCurrentLanguage()) {
            "hi" -> R.raw.medication_hindi
            "mr" -> R.raw.medication_marathi
            else -> R.raw.medication_english
        }
        ...
        mediaPlayer?.start()
        isPlaying = true
        binding.soundIc.setImageResource(R.drawable.speaker) // Change to sound-on icon
    }
